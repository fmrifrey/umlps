...
ov1.addlayer(abs(img_ts(:,:,slices2show,1))); % add underlay
ov1.addlayer(roi(:,:,slices2show).*abs(img_ts(:,:,slices2show,1))/max(abs(img_ts(:,:,slices2show,1)),[],'all'), ...
...
ov2.addlayer(abs(img_ts_unfold(:,:,slices2show,1))); % add underlay
ov2.addlayer(roi(:,:,slices2show).*abs(img_ts_unfold(:,:,slices2show,1))/max(abs(img_ts_unfold(:,:,slices2show,1)),[],'all'), ...
    'caxis',[eps,0.5],'cmap',[0;1].*[0,0,1]); % add ROI
ov2.show;
colorbar off
axis off
...
ov3.addlayer(abs(img_ts(:,:,slices2show,1))); % add underlay
...
ov4.addlayer(abs(img_ts_unfold(:,:,slices2show,1))); % add underlay
